...
public class D64 extends DiskImage {
	...
	public void readDirectory() {
		...
		try {
			...
			int filenumber = 0;
			do {
				if (track >= D64Constants.D64_TRACKS.length || sector >= getMaxSectors(track)) {
	...
	protected void writeDirectoryEntry(CbmFile cbmFile, int dirEntryNumber) {
		...
		if (entryNum > 7) {
			while (entryNum > 7) {
				...
				feedbackMessage.append("LongDirectory: "+entryNum+" dirEntrys remain, next: Track "+thisTrack+", Sector "+thisSector+"\n");
				entryNum = entryNum - 8;
			}
		}
