...
public class LNX extends DiskImage {
	...
	public final static String IMAGE_TYPE_NAME = "LNX";
	/** All LNX blocks are 254 bytes. No track/sector byte first in each block.*/
	private final static int BLOCK_SIZE = 254;
	/** The CR delimiter used in the LNX directory structure. */
	private final static int LNX_MARK = 0x0d;
	/** Number of directory blocks at start of file (using 254 bytes each */
	private int numDirBlocks = 0;
	/** The start of the first data block (at index <code>numDirBlocks * 254</code>) */
	...
	public void readDirectory() {
		...
		try {
			...
			if (count == 3 && (cbmDisk[++pos] & 0xff) == LNX_MARK) {
				// Found end of basic.
				String str;
				...
				str = new String(Arrays.copyOfRange(cbmDisk, pos, end));
				String[] dirItems = str.trim().split(" +", 2);
				numDirBlocks = Integer.parseInt(dirItems[0]);
				String diskName = dirItems.length > 1 ? dirItems[1] : "";
				dataStart = numDirBlocks * BLOCK_SIZE;
				...
				for (num = 0 ; num < fileCount && pos < cbmDisk.length; num++) {
					...
					cf.setSizeInBytes(cf.getSizeInBlocks() * BLOCK_SIZE - (lsu > 0 ?  BLOCK_SIZE - lsu + 1 : 0));
					...
					dataPos += size * BLOCK_SIZE;
					//feedbackMessage.append("cf="+cf.toString()+"\n");
				}
			} else {
	...
	private int getTypeFromLnxType(String lnxType) {
		switch (lnxType) {
		case "D": return DiskImage.TYPE_DEL;
		case "P": return DiskImage.TYPE_PRG;
		case "R": return DiskImage.TYPE_REL;
		case "S": return DiskImage.TYPE_SEQ;
		case "U": return DiskImage.TYPE_USR;
		default:  return DiskImage.TYPE_PRG;
		}
	}
